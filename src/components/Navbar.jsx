  ...
  return (
    <nav>
        ...
        <ul className='hidden md:flex gap-6 text-[18px] cursor-pointer'>
          <NavLink to='/' className='active:text-[#947458]'>Home</NavLink>
          <NavLink to='categories'>Shop</NavLink>
          <NavLink to='aboutus'>About Us</NavLink>
          <NavLink to='contactus'>Contact Us</NavLink>
        </ul>

        <button className='flex flex-row items-center right-0 z-10 font-bold justify-center text-2xl cursor-pointer transition-all duration-0.3 linear md:hidden lg:hidden text-black' onClick={() => setIsOpen((prev) => !prev)}>
        ...
        <div className='hidden md:flex gap-4'>
          ...
          className={`${isOpen ? "flex" : "hidden"} cursor-pointer w-2/4 lg:hidden flex-col fixed right-0 h-screen items-start z-10 text-[1em] space-y-6 text-black bg-gray-100`}
        >
